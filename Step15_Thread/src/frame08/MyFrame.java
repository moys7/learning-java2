...
public class MyFrame extends JFrame implements ActionListener{
   ...
   public static void main(String[] args) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		//Runnable r=()->{};아래가 이해 안되면
		//아래는 new Thread(()->{}).start 
		//new Thread(r).start안에 r을 넣은것이라 볼수 있다.
		//Runnable 인터페이스에는 run 메소드 하나만 있어서 가능
		...
		new Thread(()->{
			...
			while(true) {
				...
				if(count==0) {//만일 count가 0이면
					break;//반복문 탈출 (반복문 탈출하면 run()메소드가 리턴되기 떄문에 스레드가 종료된다.)
				}
				try {
					Thread.sleep(1000);
